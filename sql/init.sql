...
-- Propósito: bootstrap de base de datos inicial para Supabase (Auth + Postgres)
-- Requerimientos: extensiones, enums, tablas, constraints, funciones internas stub
...
-- Esquema interno para funciones sensibles
create schema if not exists internal;

-- Tabla de perfiles (derivada de auth.users)
...
-- Miembros de equipo (scoped al torneo vía FK compuesto)
...
-- Resultados de partido (un registro activo por partido)
create table if not exists public.match_results (
  ...
  set_scores jsonb, -- [{home:6, away:4}, ...]
  accepted boolean,
...
-- Eventos (para realtime / notificaciones)
...
-- Funciones internas stub (se implementan en Edge Functions, pero se dejan firmas)
...
as $$
begin
  -- Implementación real en Edge Function (generate_fixture).
  -- Aquí solo se deja firma para control de permisos.
...
-- Limpieza de audit_logs (usada por maintenance_cleanup)
