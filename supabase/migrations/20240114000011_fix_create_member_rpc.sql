-- Fix RPC function for creating members
-- 1. Drop old function to avoid signature conflicts
DROP FUNCTION IF EXISTS public.create_tenant_member(UUID, TEXT, gender, TEXT, TEXT);

-- 2. Recreate with TEXT for gender to avoid type matching issues from JS
CREATE OR REPLACE FUNCTION public.create_tenant_member(
    p_tenant_id UUID,
    p_name TEXT,
    p_gender TEXT, -- Changed from gender enum to TEXT for easier JS interop
...
GRANT EXECUTE ON FUNCTION public.create_tenant_member TO authenticated;
GRANT EXECUTE ON FUNCTION public.create_tenant_member TO service_role;
